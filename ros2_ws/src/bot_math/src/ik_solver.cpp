...
LimbJointAngles IkSolver::calculate_ik(double target_x, double target_y,
  ...
  if (A < l1) { // handle inside-radius targets
    A = std::max(A, l1);
    std::cout << "recieved target within forbidden inner radius - clamping the "
                 "boundary"
              << std::endl;
  }
  ...
  if (B < 1e-9) { // happens when target is right under hfe
    B = 1e-9;
    std::cout << "received target at planar hip singularity - clamping B away "
                 "from zero"
              << std::endl;
  }
  if (B > l2 + l3) { // target is further away then femur + tibia
    B = l2 + l3;
    std::cout << "received target at/beyond planar outer reach - saturating to "
                 "boundary pose"
              << std::endl;
  } else if (B < std::abs(l2 - l3)) { // target is too close to hfe
    B = std::abs(l2 - l3);
    std::cout << "received target at/within planar inner reach limit - "
                 "saturating to boundary pose"
              << std::endl;
  }
